...
<script type="text/javascript" charset="utf-8"
	src="plug-in/ueditor/ueditor.all.js"> </script>
...
<body>
	<t:formvalid formid="formobj" dialog="true" usePlugin="password"
		layout="table" action="noticeController.do?doUpdate" tiptype="1"
		...
		<table style="width: 100%;" cellpadding="0" cellspacing="1"
			...
			<tr>
				...
				<td class="value"><input id="noticeTitle" name="noticeTitle"
					...
					style="display: none;">通知标题</label></td>
			</tr>
			<tr>
				<td align="right"><label class="Validform_label"> 内容: </label>
				</td>
				<td class="value"><input id="noticeContent"
					name="noticeContent" type="hidden"
					value='${tSNoticePage.noticeContent}'> <script id="content"
						type="text/plain" style="width: 700px;"
						value='${tSNoticePage.noticeContent}'></script> <script
			...
			<tr>
				...
				<td class="value"><input type="radio" name="noticeType"
					value="1" datatype="*"
					<c:if test="${tSNoticePage.noticeType=='1'}">checked="checked"</c:if>
					<c:if test="${empty tSNoticePage.noticeType}">checked="checked"</c:if> />通知
					&nbsp;&nbsp;<input type="radio" name="noticeType" value="2"
					<c:if test="${tSNoticePage.noticeType=='2'}">checked="checked"</c:if> />公告
			...
			<tr>
				...
				<td class="value"><input type="radio"
					onclick="dataytpeSelect()" id="noticeLevel" name="noticeLevel"
					value="1" datatype="*"
				...
				<input type="radio" onclick="dataytpeSelect('roleName')"
					...
					<c:if test="${tSNoticePage.noticeLevel!='2'}">style="display: none"</c:if>>
						<input name="roleid" name="roleid" type="hidden"
						value="${rolesid}" id="roleid"> <input name="roleName"
						class="inputxt" value="${rolesName }" id="roleName"
						readonly="readonly" /> <t:choose hiddenName="roleid"
				...
				<input type="radio" onclick="dataytpeSelect('userName')"
					...
					<c:if test="${tSNoticePage.noticeLevel!='3'}">style="display: none"</c:if>>
						<input name="userid" name="userid" type="hidden"
						value="${usersid}" id="userid"> <input name="userName"
						class="inputxt" value="${usersName }" id="userName"
						readonly="readonly" /> <t:choose hiddenName="userid"
							hiddenid="id" url="noticeAuthorityUserController.do?selectUser"
							name="userList" icon="icon-search" title="common.user.list"
							textname="userName" isclear="true" isInit="true"></t:choose>
				</span> <span class="Validform_checktip"></span> <label
					class="Validform_label" style="display: none;">授权级别</label></td>
			</tr>
			<tr>
				<td align="right"><label class="Validform_label"
					style="white-space: nowrap;"> 阅读期限: </label></td>
				<td class="value"><input class="Wdate"
					onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" style="width: 150px"
					id="noticeTerm" name="noticeTerm" ignore="ignore"
					value="<fmt:formatDate value='${tSNoticePage.noticeTerm}' type="date" pattern="yyyy-MM-dd"/>">
					<span class="Validform_checktip"></span> <label
					class="Validform_label" style="display: none;">阅读期限</label></td>
			</tr>
		</table>
	</t:formvalid>
</body>