...
<body style="overflow-y: hidden" scroll="no">
	<t:formvalid formid="formobj" layout="div" dialog="true">
		<fieldset class="step">
			...
			<div class="form">
				<label class="form"> <%-- <t:mutiLang langKey="common.username"/> --%>操作人:${tsLog.TSUser.userName }
				</label>
			</div>
			<div class="form">
				...
				<c:if test="${tsLog.operatetype==1}">
					<t:mutiLang langKey="common.login" />
				</c:if>
				<c:if test="${tsLog.operatetype==2}">
					<t:mutiLang langKey="common.logout" />
				</c:if>
				<c:if test="${tsLog.operatetype==3}">
					<t:mutiLang langKey="common.insert" />
				</c:if>
				<c:if test="${tsLog.operatetype==4}">
					<t:mutiLang langKey="common.delete" />
				</c:if>
				<c:if test="${tsLog.operatetype==5}">
					<t:mutiLang langKey="common.update" />
				</c:if>
				<c:if test="${tsLog.operatetype==6}">
					<t:mutiLang langKey="common.upload" />
				</c:if>
				<c:if test="${tsLog.operatetype==7}">
					<t:mutiLang langKey="common.other" />
				</c:if>
			</div>
			<div class="form">
				<label class="form"> <t:mutiLang langKey="log.content" />:
				</label> ${tsLog.logcontent }
			</div>
			<div class="form">
